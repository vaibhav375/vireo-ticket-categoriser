...
- "The model: 99.9% looked perfect, but the test reused phrasings the model had seen. So I built a harder test that holds out
  ...
  agents' notes: 89%, and 94% on the tickets it's confident about." (show `docs/MODEL_IMPROVEMENT.md` table)
...
"One command, about 80 seconds, no paid calls, no LLM needed, and 91 tests; every bug fix started with a failing test. The goal: cut wrong-team tickets from 17% to 5%, about Rs 1.5 lakh a quarter.
